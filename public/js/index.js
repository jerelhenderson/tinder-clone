/*
    Business logic for user interactions with the app
*/

window.addEventListener("DOMContentLoaded", () => {
    hideLoading();

    // obtain auth'ed user's info
    ...
    if (authenticatedUser) {
        ...
        if (inputMessage) {
            ...
            error => {
                alert('Cannot send message. Please again try later');
            }
            );
        }
        };
        ...
        CometChat.addMessageListener(
            ...
            onTextMessageReceived: (message) => {
                ...
                if (selectedContact && selectedContact.uid === senderUid) {
                    ...
                    toastr.info(`There is a new message from ${message.sender.name}`);
                }
                }
            },
        ...
        CometChat.init(config.CometChatAppId, appSetting).then(
            ...
            usersRequest.fetchNext().then(
                ...
                if (userList && userList.length !== 0) {
                    ...
                    mainLeftEmpty.innerHTML = 'You do not have any contacts';
                }
                },
                error => {
                }
        ...
        const applySwing = () => {
        $(".main__card-item").on("swiperight", () => {
            swipeRight(this);
        });
        $(".main__card-item").on("swipeleft", () => {
            swipeLeft(this);
        });
        };
        ...
        if (matchRequestFrom && matchRequestTo) {
            ...
            const notificationMessage = {
                message: `Congratulations! ${authenticatedUser.name} and ${matchRequestReceiver} have been matched`,
                type: 'match',
